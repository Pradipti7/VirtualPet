...
export default function ActionButtons({ onPlay, onFeed, onAction, onGames }) {
  return (
    <div className="absolute bottom-0 left-0 right-0 pb-4 pt-8 px-4 z-20 bg-gradient-to-t from-amber-700/50 to-transparent">
      <div className="flex justify-center gap-3 max-w-md mx-auto">
        {ACTIONS.map((a) => (
          <button
            key={a.action}
            onClick={() => a.action === 'play' ? onPlay() : a.action === 'feed' ? onFeed() : a.action === 'games' ? onGames() : onAction(a.action)}
